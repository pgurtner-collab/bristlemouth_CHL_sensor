...
| Wiper | 2 out-and-back sweeps every 30 min (`chlWipeIntervalMin`, `chlWipeSweeps`) |
...
chl set chlWipeIntervalMin 60    # set one key and save it to NVM
```
...
2. **The calibration constants are unverified.** `chlCalOffsetV = 0.0235 V` and
   `chlCalScale = 25.4520 µg/L/V` were inherited from the first version of this
   app. They are the right *form* for a Turner certificate but have not been
   checked against the certificate for this sensor head. Both are runtime keys, so
   correcting them is `chl set` and no reflash.

3. **ADS1115 supply headroom.** The wiring notes say VDD comes from the 3V3 rail
   while the PGA is set to ±4.096 V. A PGA range wider than the supply is not
   reachable: the absolute maximum on an analog input is VDD + 0.3 V whatever the
   range, so a C-FLUOR output above ~3.3 V would clip while the input protection
   diodes conduct, and the top third of the sensor's 0–5 V range would be lost.
   Measure VDD at the breakout with a meter. If it is 3.3 V, either move it to 5 V
   or add a divider ahead of it. `sat_count` and the `0x20` flag will catch
   clipping in the meantime, and `chl read` shows raw counts.
